...
def login(url, username=None, password=None, session=None, cookies=None):
    ...
    while attempts > 0:
        ...
        if lgn["code"] == 1001:
            ...
            kaptcha_path = os.path.abspath("kaptcha.png")
            with open(kaptcha_path, "wb") as pic:
                pic.write(base64.b64decode(verify_data.pop("kaptcha_pic")))
            print(f"需要验证码，已保存到：{kaptcha_path}")
            if os.name == "nt":
                try:
                    os.startfile(kaptcha_path)
                except Exception:
                    pass
            else:
                print("请打开验证码图片并输入（注意区分大小写）")
            verify_data["kaptcha"] = input("请输入验证码：")

            ret = student_client.login_with_kaptcha(**verify_data)
            if ret["code"] == 1000:
                break

            print(f"验证码或登录失败：{ret.get('msg')}")
            ...
            if attempts == 1:
                ...
                run_log = ret.get("msg")
                if github_actions:
                    write_github_summary(run_log, ret.get("code"))
            time.sleep(1)
            attempts -= 1
            continue
        elif lgn["code"] != 1000:
