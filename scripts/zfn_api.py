...
class Client:
    ...
    def login(self, sid, password):
        ...
        try:
            ...
            need_verify = True
            no_cache_headers = dict(self.headers)
            ...
            latest_cookies = self.sess.cookies.get_dict()
            return {
                "code": 1001,
                "msg": "获取验证码成功",
                "data": {
                    ...
                    "cookies": latest_cookies or pre_cookies,
                    "password": password,
                    "modulus": modulus,
                    "exponent": exponent,
    ...
    def login_with_kaptcha(self, sid, csrf_token, cookies, password, modulus, exponent, kaptcha, **kwargs):
        ...
        try:
            ...
            if str(tips) != "":
                ...
                if "用户名或密码" in tips.text():
                    # 尝试使用明文密码再试一次（部分学校在验证码模式下不接受加密密码）
                    login_data["mm"] = password
                    req_login = self.sess.post(
                        self.login_url,
                        headers=self.headers,
