...
# 仅用于本地：在代码中预置环境变量（不提交到仓库）
local_env = {
    "URL": "http://jwxt.cumt.edu.cn/jwglxt/",
    "USERNAME": 'xxx',
    "PASSWORD": 'xxx',
    "TOKEN": 'xxx',
    "FORCE_PUSH_MESSAGE": "False",
    "CHECK_INTERVAL_SECONDS": "360",
    "LOCAL_COOKIES": 'JSESSIONID=xxx; route=xxx',
}
...
is_github_actions = bool(github_actions)

# 将字符串转换为布尔值
# 是否强制推送信息
# 本地运行默认不强制推送，可通过环境变量 FORCE_PUSH_MESSAGE=True 开启
...
def run_once():
    ...
    # 登录（优先使用本地 cookies，可绕过验证码）
    cookie_dict = parse_cookie_string(local_cookies)
    if cookie_dict:
        student_client = login(url, cookies=cookie_dict)
    else:
    ...
    first_run_text = (
        "你的程序运行成功\n"
        "从现在开始,程序将会每隔 6 分钟自动检测一次成绩是否有更新\n"
        "若有更新,将通过微信推送及时通知你\n"
        "------"
    )
    ...
    integrated_send_info = (
        ...
        f"{workflow_info if is_github_actions else current_time}\n"
        f"{copyright_text}"
    )
